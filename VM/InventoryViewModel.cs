...
namespace ERP.ViewModels
{
    public class InventoryViewModel : INotifyPropertyChanged
    {
        ...
        public ObservableCollection<Inventory> SelectedCollection { get; } = new ObservableCollection<Inventory>();
        ...
        public InventoryViewModel()
        {
            ...
            AddItemCommand = new RelayCommand(_ => { AddInventoryWindow add = new AddInventoryWindow(Items); add.ShowDialog(); });
            RemoveItemCommand = new RelayCommand(_ => DeleteInventory());
        ...
        private async void previousPage()
        {
            if (CurrentPageIndex > 1)
            {
                ...
                var sql = $"SELECT id, name,sku,amount FROM Inventory LIMIT 100 OFFSET {(CurrentPageIndex-2)*100}";
        ...
        private async void nextPage()
        {
            ...
            var sql = "SELECT count(id) FROM Inventory";
            ...
            if (CurrentPageIndex >= totalPages)
            {
             ...
             sql = $"SELECT id, name,sku,amount FROM Inventory LIMIT 100 OFFSET {(CurrentPageIndex) * 100}";
        ...
        private async void QueryInventory()
        {
            var connStr = "Data Source=MYDB.db";
            using var db = new ERP.Data.DatabaseClient("Microsoft.Data.Sqlite", connStr);
        ...
        private async void DeleteInventory()
        {
            ...
            // 3. 拼接 SQL
            var sql = $"DELETE FROM Inventory WHERE id IN ({string.Join(",", placeholders)});";
            ...
            {
                MessageBox.Show("Failed to delete inventory.");
            }
        }
